...
export function PlanSection() {
  return (
    <section className="bg-[#050505] py-[120px] font-almarai rtl text-right relative overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-[1440px] mx-auto px-6 flex flex-col items-center">
        ...
        >
          <h2 className="text-[48px] font-extrabold text-white mb-[16px]">إختر خطتك</h2>
          <p className="text-white/40 text-[18px] font-medium">
            يمكنك إختيار خطتك الآن بمميزات تناسب فئتك
          </p>
        </motion.div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-[24px] items-center w-full max-w-[1200px]">
          {tiers.map((tier, i) => (
            <motion.div
              key={i}
              ...
              viewport={{ once: true }}
              className={`relative rounded-[32px] p-[40px] flex flex-col transition-all duration-500 min-h-[660px] border ${
                tier.highlight 
                  ? 'bg-linear-to-b from-[#1a1426] to-[#0a0712] text-white shadow-[0_0_80px_rgba(192,132,252,0.1)] z-20 scale-[1.05] border-white/10' 
